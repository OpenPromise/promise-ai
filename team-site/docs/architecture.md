...
> 记录人：小黑；日期：2026-08-22；本阶段只产出规划文档，不写代码（Phase 2 起实现）。

## 1. 架构方案：React + SpringBoot + nginx 部署拓扑
...
- **后端**：Java SpringBoot 3（JDK 17），提供只读内容 API（新闻/角色/世界/都市），MVP 阶段数据可先用内存静态数据，后续换 DB。
- **nginx**：静态托管 + 单一路由 `/api/*` 反代到 SpringBoot；SPA 路由 fallback（`try_files ... /index.html`）。
...
├── backend/                  # Phase 3：SpringBoot
│   └── src/main/
│       ├── java/com/teamai/  # controller / service / repository / entity / dto
│       └── resources/        # application.yml / 静态初始数据
└── nginx/                    # Phase 4
    ├── nginx.conf
    └── conf.d/team-site.conf # 静态托管 + /api 反代 + SPA fallback
```
...
2. **SpringBoot 只做内容 API**：团队官网无复杂业务，后端保持"薄 API 层 + 数据"，不引安全框架（无登录）。
3. **nginx 托管静态 + 反代**：避免 Node 常驻进程，符合既有服务器运维习惯（小优管）；/api 反代是唯一动态路径。
...
3. **后端数据源**：MVP 用内存静态数据还是直接上 DB？→ 假设：MVP 静态数据，接口字段先定型。
