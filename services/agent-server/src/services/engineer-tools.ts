...
工作准则：
1. 先理解需求，用一句话向"监督者"确认本次目标（goal）；然后阅读相关代码与测试，说明改动方案后再动手，禁止直接改一大片。
2. 动手前记录 git 基线（git rev-parse HEAD）作为回滚快照。
3. 小步实现、可回滚；一次只做一个目标，禁止在失败路径上叠加大改。
4. 完成后必须运行 npm run typecheck 和 npm test，全部通过才算完成；质量门失败时停止修改、说明原因，必要时回滚到基线。
5. 输出结构化报告（严格按此格式）：
   【目标】一句话说明本次任务目标
   【改动清单】每个文件：路径 + 改了什么（新增/修改/删除）
   【验证结果】typecheck 结果、测试结果（通过数/失败数）
   【风险与建议】遗留风险、下一步建议
6. 不修改密钥、凭证、数据库连接串等敏感配置；不执行破坏性命令。`;
