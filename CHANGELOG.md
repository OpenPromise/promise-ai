...
- 新增 engineer.delegate（小黑工程师子代理），复用 dsh 底盘注入工程师人格，
  支持派单-监督-汇报工作流
...
- **`AUTO_APPROVE_ALL=true`**：所有工具（含 L2/L3：终端、电源、删除等）
  自动执行，不再弹确认；微信/桌面/无人值守任务同样生效
- 权限系统本身保留（开关可随时回退），`tool-execution` 在全权限模式下
  跳过审批直接运行
- 测试：L3 工具在全权限模式下直接执行、无 permission.request
...
- **服务器端** `scripts/deploy/sync-bot-changes.sh`：初始化服务器 git 仓库
  （基线快照），把容器 /app 里 bot 自我开发的改动同步到宿主机仓库并提交；
  `.env`、`weixin-files/` 等不入库，幂等（无改动不提交）
- **本地端** `scripts/deploy/pull-bot-changes.ps1`：SSH 拉回服务器改动并
  本地提交（base64 直传保留路径、限定提交范围）；`npm run sync:bot` 一键
- 服务器仓库已初始化：基线 + 同步提交，bot 改动从此永久保留并可回滚

## [0.14.25] - 2026-08-20

### 自主开发任意工具（发现 → 集成 → 激活闭环）

- **github.search_repos（L0）**：搜索 GitHub 仓库（星标/语言/许可证/描述），
  用于按用户要求查找可参考/集成的开源项目
- **self.apply（L1）**：激活自我开发改动——仅当本进程内 `self.check` 通过
  才允许，随后优雅重启（约 10 秒）让新工具/新代码生效，无需人工确认；
  解决此前"微信里改完代码无法激活"的死结
- **开发流程升级**（persona/self-development.md）：先判断是否真需要外部
  项目（标准算法优先原生实现）→ github.search_repos 评估（许可证/依赖）→
  coding.run 集成（薄封装不复制大段代码）→ 遵守权限准则 → self.check →
  self.apply → 告知用户如何使用
- **测试**：github 搜索、self.apply 门禁用例；全量通过
