...
## 十九、Seedance 2.5 文生视频实测受阻：ModelNotOpen 账号未开通（任务沉淀，2026-08-22）

> 记录人：小黑；记录日期：2026-08-22；依据：本次"官网素材 Phase 2（含首页视频）"任务实施结果。§十八 已确认 API 端点/参数，本次是**真实调用**的新事实。

### 1. 关键事实（均有工具结果依据）

- **【已确认】调用 `doubao-seedance-2-5-260628` 创建任务返回 HTTP 404，body 为业务错误 `{"error":{"code":"ModelNotOpen","message":"Your account <id> has not activated the model doubao-seedance-2-5-260628. Please activate the model service in the Ark Console.","type":"Not Found"}}`**——端点正确（同基地址 GET 返回 200 空任务列表），404 是"模型未开通"的业务错误，不是 URL 错。
- **【已确认】开通条件（官方教程 2607688）**：满足任一——账户余额 > 200 元 / 购买 200 元档位及以上专属节省计划 / 已购 Seedance 2.5 系列资源包有余量；均在控制台操作，纯 API 无法自行开通。
- **【已确认】`GET /api/v3/models`（Bearer 鉴权）返回平台模型目录（130 个），不区分账号是否已开通**：`doubao-seedance-2-5-260628` 在列但 status=None；"开通与否"只能通过真实创建任务（ModelNotOpen 即未开通）或控制台确认。想零成本探测开通状态：创建任务若报 ModelNotOpen 不产生费用（任务未创建）；若成功会真实计费，探测前须经授权。
- **【已确认】文生图 `doubao-seedream-5-0-pro-260628` 在本次账号可用**：7 张图（3×1024x1536 + 4×1536x1024）全部成功，单张 38-49s，2.7-3.4MB；两种尺寸均通过 size 校验（像素与宽高比区间内）。

### 2. 沉淀经验（原子化 + 置信度）

1. **【高置信·本次验证】HTTP 404 不一定是端点错：先看响应 body**。方舟视频 API 对"模型未开通"复用 404 + `type:"Not Found"`，但 `code` 是 `ModelNotOpen` 且 message 指明账号与模型——拿 body 判断根因，别看到 404 就去改 URL（同基地址 GET 200 可佐证端点无误）。
